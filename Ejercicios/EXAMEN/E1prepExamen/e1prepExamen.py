...
###################
def menuBuscarConsola(consolas,ventas):
    while True:
        busqueda=input("Ingrese Sigla del juego: ").strip().upper()
        validacion=validarSigla(busqueda)
        if not validacion:
            print("La sigla no puede estar vacia, debe tener entre 5 y 5 caracteres")
            continue
        elif validacion and not consolaEncontrada(busqueda,consolas):
            print("La consola no existe en el catalogo, reintente o ingresela")
            break
        elif validacion and consolaEncontrada(busqueda,consolas):
            detalleConsola(busqueda,consolas,ventas)
            break       
 ###################         
def detalleConsola(busqueda,consolas,ventas):
    nombre,fabricante,lanzamiento=consolas[busqueda]
    precio,stock=ventas[busqueda]
    print("====== Detalles de Consola ======")
    print("Sigla", busqueda)
    print("Nombre: ", nombre)
    ...
    print("==================================")
###################
def menuEliminarConsola(consolas,ventas):
    while True:
        eliminable=input("Ingrese Sigla del juego a eliminar: ").strip().upper()
        validacion=validarSigla(eliminable)
        if not validacion:
            print("La sigla no puede estar vacia y debe tener entre 5 y 5 caracteres")
            continue
        elif validacion and not consolaEncontrada(eliminable,consolas):
            print("La consola no existe en el catalogo, reintente o ingresela")
            break
        elif validacion and consolaEncontrada(eliminable,consolas):
            del consolas[eliminable]
            del ventas[eliminable]
            print("Consola eliminada correctamente")      
###################
def mostrarConsolas(consolas, ventas):
    counter=0
    print("==================================================")
    for sigla, datos in consolas.items():
        counter=counter+1
        nombre,fabricante,ano = datos
        precio,stock= ventas[sigla]

        print(f"{sigla} | {nombre} | {fabricante} | {ano} | ${precio} | Stock: {stock}")
    print("==================================================")
    print("Total de consolas:", counter)
...
def consolaEncontrada(busqueda,consolas):
    if busqueda in consolas:
        return True
...
while True:
    ...
    try:
        ...
        if opMenu==1:
            ...
            menuBuscarConsola(consolas,ventas)
        elif opMenu==3:
            print("=== ELIMINAR CONSOLA ===")
            menuEliminarConsola(consolas,ventas)
        elif opMenu==4:
            print("=== CATALOGO COMPLETO DE CONSOLAS ===")
            mostrarConsolas(consolas,ventas)
        elif opMenu==5:
            print("Saliendo...")
            break
        else:
